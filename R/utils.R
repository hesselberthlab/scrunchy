...
#' @param output_path path for output files. matrix.mtx.gz,
#' barcodes.tsv.gz and features.tsv.gz will be generated
...
write_matrix <- function(mat, output_path){
  ...
  readr::write_lines(colnames(mat),
                     path_join(output_path, "barcodes.tsv.gz"))

  readr::write_lines(rownames(mat),
                     path_join(output_path, "features.tsv.gz"))

  Matrix::writeMM(mat, path_join(output_path, "matrix.mtx"))

  R.utils::gzip(path_join(output_path, "matrix.mtx"))
}
...
#' @param output_path path for output files. matrix.mtx.gz, barcodes.tsv.gz and features.tsv.gz
#' will be generated at the supplied path
#'
#' @export
